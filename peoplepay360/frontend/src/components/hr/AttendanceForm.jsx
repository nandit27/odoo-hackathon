...
export default function AttendanceForm({ initialValues, employees, onSubmit, onCancel, submitting, serverError }) {
  ...
  function change(event) {
    ...
    if (event.target.name === "employeeId") { const employee = employees.find((item) => item.employeeId === event.target.value); if (employee) { next.employeeId = employee.employeeId; next.employeeName = employee.fullName; next.department = employee.department; next.workingSchedule = employee.workingSchedule; next.expectedHours = expectedHoursFor(employee.workingSchedule); } }
