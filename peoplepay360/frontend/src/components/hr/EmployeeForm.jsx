...
export default function EmployeeForm({ initialValues, managers, schedules, onSubmit, onCancel, submitting, serverError }) {
  ...
  function change(e) { setValues((current) => ({ ...current, [e.target.name]: e.target.value })); setErrors((current) => ({ ...current, [e.target.name]: "" })); }
  function submit(e) {
    e.preventDefault(); const next = {};
    Object.keys(labels).forEach((name) => { if (!String(values[name] || "").trim()) next[name] = `${labels[name]} is required.`; });
    if (values.workEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(values.workEmail)) next.workEmail = "Enter a valid work email.";
    if (values.phone && !/^\d{10}$/.test(values.phone)) next.phone = "Phone number must contain exactly 10 digits.";
  ...
  return <form onSubmit={submit} noValidate className="overflow-hidden border border-slate-200 bg-white shadow-sm">
    ...
    <div className="flex justify-end gap-3 border-t border-slate-200 bg-slate-50 px-5 py-4 sm:px-6"><button type="button" onClick={onCancel} className="rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cancel</button><button disabled={submitting} className="rounded-md bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 disabled:opacity-60">{submitting ? "Saving…" : "Save employee"}</button></div>
  </form>;
}
function Field({ name, label, type, values, errors, change, inputClass }) { const phoneProps = name === "phone" ? { inputMode: "numeric", autoComplete: "tel" } : {}; return <label className="text-sm font-medium text-slate-700">{label} <Required /><input name={name} type={type} value={values[name]} onChange={change} className={inputClass} aria-invalid={Boolean(errors[name])} {...phoneProps} /><Error text={errors[name]} /></label>; }
