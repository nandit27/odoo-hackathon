...
import { createEmployee, getEmployeeById, getEmployees, updateEmployee } from "../api/employees.js";
import { getWorkingSchedules } from "../api/workingSchedules.js";
import EmployeeForm from "../components/hr/EmployeeForm.jsx";

const emptyEmployee = { employeeId: "", firstName: "", lastName: "", workEmail: "", phone: "", department: "", position: "", managerId: "", scheduleId: "", employmentType: "", joiningDate: "", status: "Active", address: "" };
...
export default function EmployeeFormPage() {
  ...
  useEffect(() => { Promise.all([getEmployees(), getWorkingSchedules(), editing ? getEmployeeById(id) : Promise.resolve(null)]).then(([people, scheduleRows, existing]) => { setManagers(people.filter((person) => String(person.id) !== String(id))); setSchedules(scheduleRows); if (existing) setEmployee(existing); }).catch((err) => setLoadError(err.message)); }, [editing, id]);
