...
export default function TimeOffFormPage() {
  ...
  useEffect(() => { Promise.all([getEmployees(), getTimeOffTypes(), getAllocations(), editing ? getTimeOffRequestById(id) : Promise.resolve(null)]).then(([people, leaveTypes, balances, existing]) => { setEmployees(people); setTypes(leaveTypes); setAllocations(balances); if (existing) setRequest(existing); else { const requestedEmployeeId = searchParams.get("employeeId"); const employee = people.find((item) => item.id === requestedEmployeeId || item.employeeId === requestedEmployeeId); setRequest(employee ? { ...emptyRequest, employeeId: employee.employeeId, employeeName: employee.fullName } : emptyRequest); } }).catch((err) => setLoadError(err.message || "Request form could not be loaded.")); }, [editing, id, searchParams]);
